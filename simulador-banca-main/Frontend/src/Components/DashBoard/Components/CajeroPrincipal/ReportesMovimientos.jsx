        ...
        <div className="flex justify-between items-center gap-x-3">
          <div className="flex flex-col justify-center items-start">
            <div className="flex flex-row items-center gap-x-3">
              <h2 className="text-lg font-medium text-gray-800 dark:text-white">Información de movimientos</h2>
              <span className="px-3 py-1 text-xs text-blue-600 bg-blue-100 rounded-full dark:bg-gray-800 dark:text-blue-400">
                {movementsTotal} movimientos
              </span>
            </div>
            <p className="text-sm text-gray-500 m-0 p-0">Todos los movimientos realizados en el día</p>
          </div>
        </div>
                ...
                <div className="overflow-hidden border border-gray-200 dark:border-gray-700 md:rounded-lg">
                  <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead className="bg-DarkSlate dark:bg-gray-800">
                      <tr>
                        <th className="px-3 py-4 text-sm font-normal text-left rtl:text-right text-white dark:text-gray-400">
                          <span>ID Movimiento</span>
                        </th>
                        <th className="px- py-4 text-sm font-normal text-left rtl:text-right text-white dark:text-gray-400">
                          <span>Realizado por</span>
                        </th>
                        <th className="px-6 py-4 text-sm font-normal text-left rtl:text-right text-white dark:text-gray-400">
                          <span>Cliente</span>
                        </th>
                        <th className="px-4 py-3 text-sm font-normal text-left rtl:text-right text-white dark:text-gray-400">
                          <span>Saldo</span>
                        </th>
                        <th className="px-4 py-4 text-sm font-normal text-left rtl:text-right text-white dark:text-gray-400 ">
                          <span>Fecha y Hora</span>
                        </th>
                        <th className="px-4 py-4 text-sm font-normal text-left rtl:text-right text-white dark:text-gray-400 w-40 text-center">
                          <span>Acción</span>
                        </th>
                      </tr>
                    </thead>
                      ...
                      {allMovimientos.slice(firstIndex, lastIndex).map((data) => (
                        <tr key={data.id_movimiento}>
                          ...
                          <td className="px-2 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">
                            <div className="flex justify-start w-40 items-center gap-x-2">
                              <img className="object-cover w-10 h-10 rounded-full" src={userProfile} alt="" />
                              <div>
                                {data.empleado ? (
                                  <>
                                    <h2 className="font-medium text-gray-800 dark:text-white">{data.empleado}</h2>
                                    <p className="text-sm font-normal text-gray-500 dark:text-gray-400">{data.rol}</p>
                                  </>
                                ) : (
                          ...
                          <td className="px-4 py-4 text-sm text-gray-500 dark:text-gray-300 whitespace-nowrap">
                            {data.consing_em ? (
                              <div className="flex items-center gap-x-2">
                                <img className="object-cover w-10 h-10 rounded-full" src={userProfile} alt="" />
                                <div>
                                  <h2 className="font-medium text-gray-800 dark:text-white">{data.empleado_consing}</h2>
                                  <p className="text-sm font-normal text-gray-500 dark:text-gray-400">{data.rol_consing}</p>
                                </div>
                              </div>
                            ) : data.cliente ? (
                              <>
                                <h2 className="font-medium text-gray-800 dark:text-white">{data.cliente}</h2>
                                <p className="text-sm font-normal text-gray-500 dark:text-gray-400">N° {data.num_cuenta}</p>
                              </>
                            ) : (
                              <p className="text-sm font-normal text-gray-500 dark:text-gray-400">Bóveda</p>
                            )}
                          </td>
